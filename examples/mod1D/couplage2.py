...
def graphe(s,e):
    ...
    ax3.plot(xrange, diff[s:e], color=clearblue, label="derivée de Tsable en °C/s ou K/s")
...
plt.plot(meteo[:,2]*Surface_dro,label="tte la puissance reçue par l'échangeur en W", color="#f8cccc")
plt.plot(agenda_dro*meteo[:,2]*Surface_dro,label="puissance solaire reçue par l'échangeur en W lorsque le dromotherm tourne", color="yellow")
plt.plot(Pdro,label="puissance captée par le dromotherme en W", color="orange")
plt.legend()
